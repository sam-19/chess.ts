...
describe('Time control', () => {
    ...
    test('start parameters parsing', async () => {
        ...
        const gameTc = new TimeControl('G/2')
        ...
        const incrTc = new TimeControl('60+10')
        ...
        const delayTc = new TimeControl('60d10')
        ...
        const turnTc = new TimeControl('20/60+1')
        ...
        expect(timeRem).toBeLessThan(3618_950)
        expect(timeRem).toBeGreaterThan(3618_850)
        for (let i=41; i<=50; i++) {
            turnTc.moveMade(i)
        }
        expect(Math.abs(turnTc.time.remaining.w-timeRem)).toBeLessThan(5)
        const multiTc = new TimeControl('40/90 SD/30 d5')
        expect(multiTc.fields.length).toStrictEqual(3)
        expect(multiTc.fields[0].start).toStrictEqual(1)
        expect(multiTc.fields[0].end).toStrictEqual(40)
        expect(multiTc.fields[0].limit).toStrictEqual(5400)
        expect(multiTc.fields[1].start).toStrictEqual(41)
        expect(multiTc.fields[1].end).toStrictEqual(null)
        expect(multiTc.fields[1].limit).toStrictEqual(1800)
        expect(multiTc.fields[2].start).toStrictEqual(1)
        expect(multiTc.fields[2].end).toStrictEqual(null)
        expect(multiTc.fields[2].delay).toStrictEqual(5)
        const fideTc = new TimeControl('40/2, SD/30 d10')
        expect(fideTc.fields.length).toStrictEqual(3)
        expect(fideTc.fields[0].start).toStrictEqual(1)
        expect(fideTc.fields[0].end).toStrictEqual(40)
        expect(fideTc.fields[0].limit).toStrictEqual(2700)
        expect(fideTc.fields[1].start).toStrictEqual(41)
        expect(fideTc.fields[1].end).toStrictEqual(null)
        expect(fideTc.fields[1].limit).toStrictEqual(1800)
        expect(fideTc.fields[2].start).toStrictEqual(1)
        expect(fideTc.fields[2].end).toStrictEqual(null)
        expect(fideTc.fields[2].delay).toStrictEqual(5)
    })
    test('field addition', () => {
